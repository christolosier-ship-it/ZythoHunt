...
export function sampleBezier(r,steps=24){const pts=[];for(let i=0;i<=steps;i++)pts.push(cubic(r.start,r.control1,r.control2,r.end,i/steps));return pts;}
...
export function segmentIntersection(a,b,c,d){const den=(d.y-c.y)*(b.x-a.x)-(d.x-c.x)*(b.y-a.y);if(Math.abs(den)<1e-9)return false;const ua=((d.x-c.x)*(a.y-c.y)-(d.y-c.y)*(a.x-c.x))/den;const ub=((b.x-a.x)*(a.y-c.y)-(b.y-a.y)*(a.x-c.x))/den;return ua>0.02&&ua<0.98&&ub>0.02&&ub<0.98;}
