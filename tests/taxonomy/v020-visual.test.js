...
test('présentation des rayons, familles, labels et microbulles', () => {
  ...
  assert.equal(getApparentNodeRadius(byId.get('west-coast-ipa'), 'unknown', 0, 0.2), 10);
  assert.equal(getApparentNodeRadius(byId.get('west-coast-ipa'), 'unknown', 2, 1.2), 24);
  assert.equal(getApparentNodeRadius(byId.get('west-coast-ipa'), 'discovered', 2, 1.2), 36);
