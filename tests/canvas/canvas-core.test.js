...
import { fitBounds, availableViewport } from '../../src/canvas/coordinate-transform.js';

test('fitBounds respecte les insets des overlays',()=>{const bounds={minX:0,minY:0,maxX:100,maxY:100,width:100,height:100}; const size={width:400,height:400}; const noInset=fitBounds(bounds,size,0); const inset=fitBounds(bounds,size,0,{min:.25,max:10},{top:100,bottom:100,left:20,right:20}); assert(inset.scale < noInset.scale); const area=availableViewport(size,{top:100,bottom:100,left:20,right:20}); assert.equal(area.height,200); assert.equal(area.centerY,200);});
