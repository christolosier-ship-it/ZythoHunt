...
}; Object.values(controls).forEach(b=>nav.append(b)); container.append(nav); controls.zoomIn.onclick=handlers.onZoomIn; controls.zoomOut.onclick=handlers.onZoomOut; controls.recenter.onclick=handlers.onRecenter; controls.motion.onclick=handlers.onToggleMotion; return { element: nav, controls, setReducedMotion(value){ controls.motion.setAttribute('aria-pressed', String(Boolean(value))); controls.motion.classList.toggle('is-active', Boolean(value)); } }; }
