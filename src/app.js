...
let map; let debug; let data; let revealBar; let toolbar; const discovery = createDiscoveryController();
...
function renderAndSync({ fit = false, focusId = null } = {}) { shell.updateMeasurements(); const mapState = map.render(appState.discovered, appState.presentation, { onSelect: selectStyle }); if (!appState.presentation.selectedId) setLive(`${mapState.visibleNodes.length} nœuds visibles — ${mapState.unknownStyleNodes.length} styles inconnus.`); debug?.sync(mapState, appState.presentation, { appVersion: APP_VERSION, taxonomyVersion: TAXONOMY_VERSION }); if (focusId) { map.focusNode(focusId); map.render(appState.discovered, appState.presentation, { onSelect: selectStyle }); } else if (fit) { map.fitState(mapState); map.render(appState.discovered, appState.presentation, { onSelect: selectStyle }); } return mapState; }
function previewReveal(id) { const node = data.nodes.find(n => n.id === id); if (!node || node.functionalType !== 'capturable') return; map.focusNode(id); discovery.preview(id, { onStart: revealId => { appState.presentation.revealPendingId = revealId; appState.presentation.selectedId = null; renderAndSync(); setLive(`Révélation : ${node.name}.`); }, onName: () => renderAndSync(), onFinish: revealId => { appState.presentation.revealPendingId = null; appState.discovered.add(revealId); appState.presentation.selectedId = revealId; renderAndSync({ focusId: revealId }); setLive(`Style révélé : ${node.name}.`); } }, { reducedMotion: appState.presentation.reducedMotion }); }
...
try {
  ...
  setReducedMotion(prefersReduced); renderAndSync({ fit: true });
