...
export function getWorldRadius(node, state = 'unknown') {
  if (node.visualLevel === 'root' || node.id === 'beer') return 190;
  if (node.visualLevel === 'fermentation' || node.nodeType === 'fermentation') return 125;
  if (node.visualLevel === 'family' || node.nodeType === 'family') return 92;
  if (state === 'selected' || state === 'reveal-pending') return 68;
  if (state && state !== 'unknown') return 62;
  return 48;
}
